...
int gethostbyaddr(char &addr[], int len, int type);
int inet_addr(char &addr[]); 
int inet_ntoa(int addr );
int ioctlsocket(int socket, int cmd, int &argp[]);
int setsockopt(int socket, int level, int optname, int& optval[], int optlen); 
#import
...
struct socket_socckaddr
{
	ushort sa_family;
	char sa_data[14];
};

struct socket_in_addr {
    ulong S_addr;
};
...
struct socket_sockaddr_in {
        short   sin_family;
        ushort sin_port;
        socket_in_addr sin_addr;
        char    sin_zero[8];
};

ulong inet_addr(ulong d, ulong c, ulong b, ulong a)
{
...
#import "msvcrt.dll"
  ...
  int memcpy(socket_sockaddr_in &dst,  socket_sockaddr_in &src, int cnt);
#import
...
int struct2int(int &str[], int field) {
  bool n=field<0;
  if (n) field=-field;
  int l=field>>16;
  if (l==0) return(0);
...
string struct2str(int &str[], int field) {
string   r="";
int l=field>>16;
if (l==0) return(r);
field&=0xFFFF;
...
void int2struct(int& str[], int field, int value) {
  ...
  str[p]&=0xFFFFFFFF>>((4-o)<<3);
  str[p]|=value<<(o<<3);
  if (o+l>4){
    p++;
    str[p]&=0xFFFFFFFF<<((o+l-4)<<3);
    str[p]|=value>>((8-o-l)<<3);
  }  
}

void str2struct(int& str[], int field, string value) {
  int l=field>>16;
  if (l<2) return;
  field&=0xFFFF;
  int p=field>>2;
  ...
  for (int i=0; i<StringLen(value); i++) {
    str[p]&=~(0xFF<<(o<<3));
    str[p]|=StringGetChar(value,i)<<(o<<3);
    if (o==3) {
      p++;
      o=0;
    } else
